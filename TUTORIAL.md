# TUTORIAL — 이 폴더 구조, 처음부터 끝까지

폴더 구조를 한 번도 안 다뤄봤어도 괜찮다. 이 문서 하나면 **각 폴더가 뭔지, 요청이 어떻게 흐르는지, 새 기능을 어디에 넣는지**를 다 안다.
예제로 "주문(Order)" 도메인이 이미 들어 있으니, 그걸 따라가며 배우면 된다.
...
이걸 **헥사고날 아키텍처**(= 포트 & 어댑터)라고 부른다. 이 템플릿은 해커톤 속도를 위해 **꼭 필요한 부분만 남긴 "라이트" 버전**이다.
...
- **주방(domain + application)** = 요리하는 곳. 진짜 가치(음식)가 여기서 만들어진다. 손님이 웹으로 주문하든 전화로 하든 **주방은 신경 안 쓴다.**
- **포트(ports)** = 주방이 외부에 거는 "주문서 양식". "이런 형식으로 주문이 들어온다/재료는 이렇게 받는다"는 **계약**.
- **어댑터(adapters)** = 양식을 실제 세상에 연결하는 코드.
  - **inbound(들어오는 쪽)** = 손님 창구(웹 API). 손님 요청을 주방이 아는 말로 바꿔 전달.
  - **outbound(나가는 쪽)** = 창고(저장소). 주방이 "재료 꺼내와" 하면 실제 냉장고(메모리/DB)에서 꺼내옴.

**핵심:** 창구를 웹에서 모바일앱으로 바꾸거나, 냉장고를 메모리에서 진짜 DB로 바꿔도 — **주방 레시피(비즈니스 로직)는 한 줄도 안 바뀐다.** 그게 이 구조의 전부다.
...
│
├── src/app/                # ← 실제 코드. 패키지 이름은 "app" 으로 고정
│   │
...
| 앱 실행 | `uv run fastapi dev src/app/main.py` (→ `/docs`) |
| 테스트 | `uv run pytest` |
