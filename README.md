...
FastAPI 기반 **헥사고날-라이트(hexagonal-lite)** 파이썬 프로젝트 골격.
해커톤마다 GitHub **"Use this template"** 로 새 레포를 찍어 쓰면, 5명 모두 *항상 같은 구조*로 시작한다.

> 폴더 구조가 처음이면 **먼저 [TUTORIAL.md](TUTORIAL.md) 를 읽어라** — 폴더 하나하나, 요청이 흐르는 길, 기능 추가법까지 설명한다.
...
```powershell
uv run fastapi dev src/app/main.py      # -> http://127.0.0.1:8000/docs (대화형 API 문서)
```
...
| 목적 | 명령 |
|---|---|
| 실행 | `uv run fastapi dev src/app/main.py` |
| 테스트 | `uv run pytest` |
